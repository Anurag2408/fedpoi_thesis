"""
client.py  (Ablation 2 — Fixed ε = 1.0 for all clients)
------------------------------------------------------------------
Federated Learning Client with DP-SGD — Fixed epsilon variant.

Changes from Phase 3:
  - self.epsilon fixed to 1.0 for ALL clients (no adaptive allocation)
  - dp_sgd_step() remains active (DP noise is ON)
  - Results saved to results/ablation2_fixedeps/

Everything else (LSTM, OSM categories, FedAvg, PrivacyAccountant) is
identical to Phase 3 to ensure a controlled comparison.

Usage:
    python experiments/ablation2_fixedeps/client.py <client_id>
...
class POILSTMClient(fl.client.NumPyClient):
    ...
    def __init__(self, client_id: int):
        ...
        # ── Fixed DP budget (Ablation 2) ──────────────────────────────────────
        #
        # All clients use the same ε = 1.0 regardless of check-in volume.
        # This is the controlled comparison against Phase 3's adaptive allocation.
        self.epsilon = 1.0                               # ← FIXED for all clients
        ...
        print(f'\n[Client {client_id}] DP Configuration (Fixed ε):')
        print(f'  Fixed ε      : {self.epsilon:.4f}  (uniform across all clients)')
    ...
    def fit(self, parameters, config):
        ...
        for epoch in range(TRAIN_CONFIG['local_epochs']):
            ...
            for batch in self.train_loader:
                ...
                venue_seq = venue_seq.to(self.device)
                time_seq  = time_seq.to(self.device)
                cat_seq   = cat_seq.to(self.device)
                targets   = targets.squeeze(1).to(self.device)

                logits = self.model(venue_seq, time_seq, cat_seq)
                ...
                # dp_sgd_step handles zero_grad, backward, clip, noise, and step.
                # Do NOT add any extra optimizer calls after this.
    ...
    def evaluate(self, parameters, config):
        ...
        print(f'  Privacy: ε_spent={epsilon_spent:.4f}  (fixed ε={self.epsilon:.4f})')
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print('Usage: python experiments/ablation2_fixedeps/client.py <client_id>')
        sys.exit(1)
