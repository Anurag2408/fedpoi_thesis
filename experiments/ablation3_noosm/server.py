"""
server.py  (Ablation 3 — No OSM Category Features)
------------------------------------------------------------------
Federated Learning Server — No OSM Ablation.

Identical to Phase 3 server but saves results to ablation3_noosm/.
Aggregates epsilon statistics from clients (DP is still active).
"""
...
class SaveModelStrategy(fl.server.strategy.FedAvg):
    """FedAvg strategy for Ablation 3 (No OSM)."""
    ...
    def aggregate_evaluate(
        ...
        print(f'\n[Server] Round {server_round} Results (No OSM):')
...
def start_server(n_rounds: int = 10, n_clients: int = 10, resume: bool = False):
    print('='*60)
    print('  FEDERATED POI RECOMMENDATION — ABLATION 3 (NO OSM)')
    print('='*60)
    print(f'\n  Rounds  : {n_rounds}')
